...
export default function UsersTable({
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault()
        router.push(`/admin/users?search=${searchTerm}&role=${currentRole}&page=1`)
    }

    const handleFilterChange = (role: string) => {
        router.push(`/admin/users?search=${searchQuery}&role=${role}&page=1`)
    }
                                ...
                                <tr key={user.id} className="hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <select
                                            ...
                                            disabled={loading === user.id || user.role === 'admin'} // Prevent changing own admin role easily
            ...
            {total > 50 && (
                <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800/50">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => router.push(`/admin/users?search=${searchQuery}&role=${currentRole}&page=${currentPage - 1}`)}
