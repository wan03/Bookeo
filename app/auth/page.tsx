...
export default function AuthPage() {
    ...
    const getAuthEmail = () => {
        ...
        // Generate synthetic email for phone auth
        ...
        try {
            const { data: { user }, error: signUpError } = await supabase.auth.signUp({
                ...
                options: {
                    data: {
                        full_name: fullName,
                        phone_number: cleanPhone // Save phone in metadata too just in case
                    }
                }
            })
            ...
            if (user) {
                ...
                const { error: profileError } = await supabase.from('profiles').insert({
                    id: user.id,
                    email: finalEmail, // We save the synthetic email for phone users, or real email
                    full_name: fullName,
                    phone_number: cleanPhone, // Save real phone number
                    role: role
                })
    ...
    const handleSignIn = async (e: React.FormEvent) => {
        ...
        const finalEmail = getAuthEmail()
        console.log('Sign in email:', finalEmail)
            ...
            if (user) {
                ...
                if (redirectTo && redirectTo !== '/') {
                    ...
                    if (userRole === 'business_owner' || userRole === 'staff') {
                        ...
                        console.log('Redirecting to influencer')
